...
.pack_and_function_check <- function(
    ...
    if( ! base::all(tempo.log)){
        tempo.cat <- base::paste0("ERROR IN THE CODE OF THE ", external.function.name, " OF THE saferGraph PACKAGE.\nTHE STRING IN fun ARGUMENT MUST CONTAIN \"::\":\n", base::paste(fun[ ! tempo.log], collapse = "\n"))
    ...
    if( ! base::all(pkg.log)){
        ...
        tempo.cat <- base::paste0(
            "ERROR IN ", 
            external.function.name, 
            " OF THE saferGraph PACKAGE. REQUIRED PACKAGE", 
    ...
    if( ! base::all(fun.log)){
        ...
        tempo.cat <- base::paste0(
            "ERROR IN ", 
            external.function.name, 
            " OF THE saferGraph PACKAGE. REQUIRED FUNCTION",
