...
.pack_and_function_check <- function(
    ...
    if( ! all(tempo.log)){
        tempo.cat <- paste0("ERROR IN THE CODE OF THE ", external.function.name, " OF THE cuteGraph PACKAGE.\nTHE STRING IN fun ARGUMENT MUST CONTAIN \"::\":\n", paste(fun[ ! tempo.log], collapse = "\n"))
    ...
    if( ! all(pkg.log)){
        ...
        tempo.cat <- paste0(
            "ERROR IN ", 
            external.function.name, 
            " OF THE cuteGraph PACKAGE. REQUIRED PACKAGE", 
    ...
    if( ! all(fun.log)){
        ...
        tempo.cat <- paste0(
            "ERROR IN ", 
            external.function.name, 
            " OF THE cuteGraph PACKAGE. REQUIRED FUNCTION",
